...
import { env, OR_KEY, OR_URL, OR_MODEL, OR_FALLBACK_MODEL, UPLOADPOST_KEY, MANYCHAT_KEY, N8N_WEBHOOK_URL } from '@/lib/config';
...
// ============================================================
//  HIKERAPI MODULE — REMOVED
//  All hiker* functions are stubs returning empty/null.
//  TODO: replace with Zernio or ScrapingBee
// ============================================================

export async function hikerGetUser(_username: string): Promise<any> { return null; }
export async function hikerGetUserId(_username: string): Promise<string | null> { return null; }
export async function hikerGetFollowers(_userId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetFollowing(_userId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetMedias(_userId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetMediaByCode(_code: string): Promise<any> { return null; }
export async function hikerGetComments(_mediaId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetLikers(_mediaId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetStories(_userId: string): Promise<any[]> { return []; }
export async function hikerSearchUsers(_query: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetHighlights(_userId: string): Promise<any[]> { return []; }
export async function hikerGetBalance(): Promise<{ requests: number; amount: number }> { return { requests: 0, amount: 0 }; }
export async function hikerGetCommentsGraphQL(_mediaId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetClips(_userId: string, _amount?: number): Promise<any[]> { return []; }
export async function hikerGetUserAbout(_userId: string): Promise<any> { return null; }
...
export async function upPublishPhotos(photos: Buffer[], title: string, platforms: string[], options?: any): Promise<any> {
  try {
    ...
    form.append('title', title);
    platforms.forEach(function(p) { form.append('platform[]', p); });
    photos.forEach(function(photo) { var ab = photo.buffer.slice(photo.byteOffset, photo.byteOffset + photo.byteLength) as ArrayBuffer; form.append('photos[]', new Blob([ab]), 'photo.jpg'); });
    if (options && options.description) form.append('description', options.description);
    if (options && options.scheduled_date) form.append('scheduled_date', options.scheduled_date);
    if (options && options.first_comment) form.append('first_comment', options.first_comment);
...
export async function upPublishVideo(videoBuffer: Buffer, title: string, platforms: string[], options?: any): Promise<any> {
  try {
    var form = new FormData();
    form.append('user', UP_PROFILE);
    var vab = videoBuffer.buffer.slice(videoBuffer.byteOffset, videoBuffer.byteOffset + videoBuffer.byteLength) as ArrayBuffer; form.append('video', new Blob([vab]), 'video.mp4');
    form.append('title', title);
    platforms.forEach(function(p) { form.append('platform[]', p); });
    if (options && options.description) form.append('description', options.description);
    if (options && options.scheduled_date) form.append('scheduled_date', options.scheduled_date);
...
export async function igSendDM(sessionid: string, csrftoken: string, recipientId: string, message: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    if (!threadRes) return { success: false, error: 'Sem resposta ao criar thread' };
...
export async function igUploadPost(sessionid: string, csrftoken: string, imageData: Buffer, caption: string): Promise<{ success: boolean; mediaId?: string; error?: string }> {
  try {
    ...
    var iab = imageData.buffer.slice(imageData.byteOffset, imageData.byteOffset + imageData.byteLength) as ArrayBuffer; formData.append('photo', new Blob([iab], { type: 'image/jpeg' }), 'photo.jpg');
...
export function parseCommand(text: string): CommandResult {
  ...
  // === HIKERAPI COMMANDS (removed — use Zernio or ScrapingBee) ===
