...
export async function GET(request: Request) {
  ...
  try {
    ...
    var cycleResults: any = await tikTokAutoCycle();
    ...
    await db.automationLog.create({
      data: {
        ...
        platform: 'tiktok',
        status: ((dmResults as any).errors?.length === 0 && (cycleResults as any).errors?.length === 0) ? 'success' : 'partial',
        result: JSON.stringify({
          dmsSource: dmResults.source,
          newDMs: dmResults.newMessages,
          autoReplied: dmResults.autoReplied,
          contentGenerated: !!cycleResults.content,
