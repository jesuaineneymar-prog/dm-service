...
export async function monitorTikTokDMs() {
    ...
    try {
      ...
      if (zernioRes.success && zernioRes.conversations && zernioRes.conversations.length > 0) {
        ...
        for (var i = 0; i < zernioRes.conversations.length; i++) {
          ...
          var { igfbGetMessages } = await import('@/lib/tiktok-engine');
          var msgRes: any = await igfbGetMessages(convId, 5);
          ...
          for (var mi = messages.length - 1; mi >= 0; mi--) {
            ...
            if (prospect && messageText.length > 0) {
              ...
              var sendRes: any = await tiktokSendDMViaZernio(convId, accountId, aiReply);
  ...
  if (results.newMessages === 0 && MANYCHAT_KEY) {
    try {
      ...
      var convRes = await tiktokGetConversations('tiktok', 20);
      ...
      for (var i = 0; i < conversations.length; i++) {
        ...
        var dmResult = await tiktokSendDM({ platform: 'tiktok', recipientId: String(conv.participant?.id || conv.id), message: aiReply });
        if (dmResult.success) {
          results.autoReplied++;
          if (prospect) {
            await db.message.create({ data: { prospectId: prospect.id, direction: 'outbound', content: aiReply, platform: 'tiktok' } });
  ...
  try {
    ...
    if (action === 'send_dm') {
      ...
      if ((ZERNIO_TT_KEY || ZERNIO_KEY) && body.conversationId && body.accountId) {
        var zResult: any = await tiktokSendDMViaZernio(String(body.conversationId), String(body.accountId), body.message);
        if (zResult.success) return NextResponse.json({ ...zResult, via: 'zernio' });
      }
      // Fallback ManyChat
      var result = await tiktokSendDM({ platform: 'tiktok', recipientId: String(body.recipientId), message: body.message, buttonText: body.buttonText, buttonUrl: body.buttonUrl });
    ...
    if (action === 'get_conversations') {
      ...
      // Fallback ManyChat
      var convs = await tiktokGetConversations('tiktok', body.limit || 50);
      return NextResponse.json(convs);
    }

    if (action === 'set_welcome') {
      if (!body.message) return NextResponse.json({ success: false, error: 'Mensagem necessaria' });
      var welcome = await tiktokSetWelcomeMessage('tiktok', body.message);
      return NextResponse.json(welcome);
    }

    if (action === 'get_profile') {
      var profile = await tiktokGetProfileInfo('tiktok');
    ...
    if (action === 'trigger_flow') {
      ...
      var flow = await tiktokTriggerFlow({ platform: 'tiktok', recipientId: String(body.recipientId), flowId: String(body.flowId) });
