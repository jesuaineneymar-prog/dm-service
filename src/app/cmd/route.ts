...
export async function POST(request: Request) {
  ...
  if (action === 'hiker_search') {
    ...
    var searchResults = await hikerSearchUsers(searchQ, 20);
    return NextResponse.json({
      success: true,
      type: 'hiker_search',
      total: searchResults.length,
      data: searchResults.map(function(u: any) {
        return { username: u.username, fullName: u.full_name, followers: u.follower_count, isPrivate: u.is_private, isVerified: u.is_verified, pk: u.pk };
      }),
    });
  }
  ...
  if (action === 'up_comments') {
    ...
    var upComments = await upGetCommentsFn(mediaIdUp);
    return NextResponse.json(upComments);
  }
  ...
  if (action === 'up_accounts') {
    ...
    var allPlatforms = ['instagram', 'facebook', 'tiktok', 'youtube', 'linkedin', 'x', 'threads', 'discord', 'telegram'];
    for (var pi = 0; pi < allPlatforms.length; pi++) {
      var pkey = allPlatforms[pi];
      var entry = sa[pkey];
