...
import { HIKERAPI_KEY, IG_USERNAME } from '@/lib/config';
...
async function importProspects(platform: string, source: string, query: string) {
  ...
  if (source === 'followers') {
    // Fetch main account followers
    var mainUser: any = await hikerFetch('/users/by/username?username=' + IG_USERNAME);
    var userId = mainUser.pk || mainUser.id;
    var followersRes: any = await hikerFetch('/users/' + userId + '/followers?count=50');
    users = followersRes.items || followersRes.data || followersRes.users || followersRes || [];
  } else if (source === 'search') {
    if (!query) throw new Error('Query de pesquisa necessaria para importar por busca');
    var searchRes: any = await hikerFetch('/users/search?query=' + encodeURIComponent(query) + '&count=50');
    users = searchRes.items || searchRes.data || searchRes.users || searchRes || [];
  } else {
    throw new Error('Fonte desconhecida: ' + source + '. Usa "followers" ou "search"');
  }
  ...
  for (var i = 0; i < users.length; i++) {
    ...
    var prospect = await db.prospect.findFirst({ where: { platform: platform, username: username } });
    if (prospect) {
      prospect = await db.prospect.update({
        where: { id: prospect.id },
        data: {
          displayName: u.full_name || undefined,
          followers: u.follower_count || undefined,
          bio: u.biography || undefined,
          avatarUrl: u.profile_pic_url || u.hd_profile_pic_url_info?.url || undefined,
          externalId: String(u.pk || u.id || ''),
        },
      });
    } else {
      prospect = await db.prospect.create({
        data: {
          ...
          avatarUrl: (u.profile_pic_url || u.hd_profile_pic_url_info?.url || ''),
          externalId: String(u.pk || u.id || ''),
        },
      });
    }
  ...
  return { imported: imported.length, prospects: imported };
}
